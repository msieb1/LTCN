...
def main(args):
    ...
    for file in files:
        ...
        delta_eulers = get_delta_euler(file) 
        i = 0
        ...
        for i in range(0, len(reader)-1):
            ...
            if USE_CUDA:
              state_embedding, state_embedding_pred, a_inv_predi, _ = tcn(torch.Tensor(frames[None]).cuda(), torch.Tensor(delta_euler[None]).cuda())
            else:
              state_embedding, state_embedding_pred, a_inv_pred, _ = tcn(torch.Tensor(frames[None]), torch.Tensor(delta_euler[None]))         
            embeddings_episode_buffer.append(state_embedding.detach().cpu().numpy())
