...
from models.pose_predictor_quat import define_model
...
from utils.network_utils import loss_rotation, loss_euler_reparametrize, loss_axisangle, batch_size, apply,\
                                    loss_quat, loss_quat_single, euler_XYZ_to_reparam, geodesic_dist_quat, loss_quat_huber
from utils.plot_utils import plot_mean
...
class Trainer(object):
    def __init__(self, use_cuda, load_model, model_folder, train_directory, validation_directory, builder, loss_fn, args, multi_gpu=True):
        ...
        validation_builder = builder(self.args.n_views, validation_directory, IMAGE_SIZE, self.args, sample_size=int(SAMPLE_SIZE / 2.0))
        ...
        for epoch in range(self.args.start_epoch, self.args.start_epoch + self.args.epochs):
            ...
            for _ in range(0, 1):
                ...
                for minibatch in data_loader:
                    ...
                    correct += np.sum(dist.data.cpu().numpy() < 0.2)
            ...
            self.writer.add_scalar('data/train_correct', correct / len(dataset), self.itr)
            self.itr += 1  
