...
from utils.util import (SingleViewDepthTripletLabelBuilder, distance, Logger, ensure_folder, collate_fn)
from utils.vocabulary import Vocabulary
from tcn import define_model_depth, DenseClassifier
...
def get_args():
    ...
    parser.add_argument('--model-folder', type=str, default=EXP_DIR + 'trained_models/depth_sv_labels')
...
args = get_args()
print(args)
builder = SingleViewDepthTripletLabelBuilder
logger = Logger(args.log_file)

def batch_size(epoch, max_size):
    exponent = epoch // 100
    return min(max(2 ** (exponent), 2), max_size)
validation_builder = builder(args.view, args.validation_directory, args.validation_directory_depth, args.labels_validation_directory, IMAGE_SIZE, args, sample_size=150)
validation_set = [validation_builder.build_set() for i in range(1)]
...
def main():
    ...
    triplet_builder = builder(args.view, \
        args.train_directory, args.train_directory_depth, args.labels_train_directory, IMAGE_SIZE, args, sample_size=150)
