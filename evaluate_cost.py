...
def feature_selection(roi_features):
    # assumes ordered by dict keys (1,2,3,...) which correspond to class labels (see in RCNN config for class names)
    roi_features_normalized = np.mean(roi_features, axis=0) / np.std(roi_features, axis=0)

    roi_features_1 = roi_features[:, 0]
    roi_features_2 = roi_features[:, 1]
    roi_features_1_map_flat = np.reshape(roi_features_1, [-1, 7*7, 256])
    roi_features_2_map_flat = np.reshape(roi_features_2, [-1, 7*7, 256])

    roi_features_1_map_mean = np.mean(roi_features_1_map_flat, axis=1)
    roi_features_2_map_mean = np.mean(roi_features_2_map_flat, axis=1)
    roi_features_1_map_max = np.max(roi_features_1_map_flat, axis=1)
    roi_features_2_map_max = np.max(roi_features_2_map_flat, axis=1)

    std_1 = np.std(roi_features_1_map_mean, axis=0).flatten()
    std_2 = np.std(roi_features_2_map_mean, axis=0).flatten()
    std_1_max = np.std(roi_features_1_map_max, axis=0).flatten()
    std_2_max = np.std(roi_features_2_map_max, axis=0).flatten()
    # set_trace()
    sel_idx_1 = np.argpartition(-std_1, FEAT_DIM)[:FEAT_DIM]
    sel_idx_2 = np.argpartition(-std_2, FEAT_DIM)[:FEAT_DIM] 
    sel_idx_max_1 = np.argpartition(-std_1_max, FEAT_DIM)[:FEAT_DIM]
    sel_idx_max_2 = np.argpartition(-std_2_max, FEAT_DIM)[:FEAT_DIM] 

    sel_features_1 = std_1[sel_idx_1]
    sel_features_2 = std_2[sel_idx_2]
    return sel_idx_1, sel_idx_2, sel_idx_max_1, sel_idx_max_2

def apply_feature_selection_episode(roi_features, sel_idx_1, sel_idx_2, sel_idx_max_1, sel_idx_max_2):
  roi_features_1 = roi_features[:, 0]
  roi_features_2 = roi_features[:, 1]
  roi_features_1_map_flat = np.reshape(roi_features_1, [-1, 7*7, 256])
  roi_features_2_map_flat = np.reshape(roi_features_2, [-1, 7*7, 256])
  ...
  feat_visual_1 = roi_features_1_map_mean[:, sel_idx_1]
  feat_visual_2 = roi_features_2_map_mean[:, sel_idx_2]

  feat_visual_max_1 = roi_features_1_map_max[:, sel_idx_max_1]
  feat_visual_max_2 = roi_features_2_map_max[:, sel_idx_max_2]

  # return np.concatenate([feat_visual_1, feat_visual_max_1, feat_visual_2, feat_visual_max_2], axis=1)
  return np.concatenate([feat_visual_2, feat_visual_max_2], axis=1)
  # return np.concatenate([feat_visual_max_2], axis=1)

def apply_feature_selection_step(roi_features, sel_idx_1, sel_idx_2, sel_idx_max_1, sel_idx_max_2):
  ...
  feat_visual_max_1 = roi_features_1_map_max[sel_idx_max_1]
  feat_visual_max_2 = roi_features_2_map_max[sel_idx_max_2]
  return np.concatenate([feat_visual_1, feat_visual_max_1, feat_visual_2, feat_visual_max_2])


if __name__ == '__main__':
    set_trace()
    parser = argparse.ArgumentParser()
    parser.add_argument('--target-path', type=str, default='embeddings_live')
    parser.add_argument('--query-path', type=str, default='embeddings_live/0_view0_emb.npy')
    parser.add_argument('--output-path', type=str, default=OUTPUT_FOLDER)
    parser.add_argument('--model-path', type=str, default=TCN_MODEL_PATH)
